...
def parse_options():
    ...
    parser.add_argument('--useful-facts-file', dest='useful_facts_file',
                        help='Useful facts of the initial state are written to this file.')
...
def main():
    ...
    build_dir = os.path.join(get_project_root_path(), 'builds', 'powerlifted-debug' if options.debug else 'powerlifted-release', 'bin')

    if options.build:
        from src.powerlifted.build import build
    ...
    if options.useful_facts_file:
        CPP_EXTRA_OPTIONS += ['--useful-facts-file', options.useful_facts_file]
