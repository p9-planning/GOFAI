...
# data taken from this revision in https://gitlab.com/Fareces/planning_newfeatures
DATA_REVISION = "2f57e8d227f77475f9bc7cca7d630f450787c8d3"

BUILD_OPTIONS = ["release", "powerlifted"]
DRIVER_OPTIONS = ["--alias", "lama-first"]
USEFUL_FACTS_OPTIONS = ["--compute-relaxed-info", "powerlifted-facts"]
...
EMBEDDING_FOLDER = "/proj/decoupled-search/ml_models/embeddings/"
CLASSIFIERS_FOLDER = f"/proj/decoupled-search/ml_models/classifiers-restructured/{common.CLASSIFIERS[DOMAIN]}/"

CONFIGS = [("fasttext-opt",  ["--translate-options", "--grounding-action-queue-ordering", "fasttext", 
                              "--relaxed-plan-file", "useful_facts",
                              "--trained-model-folder", CLASSIFIERS_FOLDER,
                              "--fasttext-model", EMBEDDING_FOLDER, "--optimized-fasttext"]),

           ("RR-ft-opt",   ["--translate-options", "--grounding-action-queue-ordering", "roundrobinfasttext",
                            "--relaxed-plan-file", "useful_facts",
                            "--trained-model-folder", CLASSIFIERS_FOLDER,
                            "--fasttext-model", EMBEDDING_FOLDER, "--optimized-fasttext"]),

           ("ratio-ft-opt", ["--translate-options", "--grounding-action-queue-ordering", "ratiofasttext",
                             "--action-schema-ratios", RATIO_FILE,
                             "--relaxed-plan-file", "useful_facts",
                             "--trained-model-folder", CLASSIFIERS_FOLDER,
                             "--fasttext-model", EMBEDDING_FOLDER,
                             "--plan-ratios", "--optimized-fasttext"]),
]
...
for name, config in CONFIGS:
    exp.add_algorithm(name, REPO, REVISION, config + TERMINATION_CONDITION, build_options=BUILD_OPTIONS, driver_options=DRIVER_OPTIONS + USEFUL_FACTS_OPTIONS)
    exp.add_algorithm(f"{name}-inc", REPO, REVISION, config, build_options=BUILD_OPTIONS, driver_options=DRIVER_OPTIONS + USEFUL_FACTS_OPTIONS + INCREMENTAL_GROUNDING)
