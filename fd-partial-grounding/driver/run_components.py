...
def run_translate(args, incremental_grounding = False):
    ...
    if incremental_grounding:
        stdout, stderr, returncode = call.get_output_and_returncode(
            "translator",
            cmd,
            time_limit=time_limit,
            memory_limit=memory_limit)
        num_acts_regex = re.compile(r'^(\d+) actions instantiated')
        unsolv_regex = re.compile(r'No relaxed solution! Generating unsolvable task...')
        number_actions_instantiated = -1
        translator_unsolvable = False
        for l in stdout.splitlines():
            print(l, file=sys.stdout)
            if (number_actions_instantiated == -1):
                match = num_acts_regex.search(l)
                if (match):
                    number_actions_instantiated = int(match.group(1))
            if (not translator_unsolvable):
                match = unsolv_regex.search(l)
                if (match):
                    translator_unsolvable = True
    else:
    ...
    if returncode == 0:
        if incremental_grounding:
            if translator_unsolvable:
                return (returncodes.TRANSLATE_UNSOLVABLE, True, number_actions_instantiated)
            else:
                return (0, True, number_actions_instantiated)
        return (0, True)
    elif returncode == 1:
        # Unlikely case that the translator crashed without raising an
        # exception.
        if incremental_grounding:
            return (returncodes.TRANSLATE_CRITICAL_ERROR, False, number_actions_instantiated)
        ...
        if incremental_grounding:
            return (returncode, False, number_actions_instantiated)
...
def run_search(args):
    ...
    plan_manager.delete_existing_plans(args.keep_first_plan_file)
