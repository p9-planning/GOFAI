...
def compute_model(prog):
    ...
    all_operators_file = open('all_operators', 'w')
    with timers.timing("Computing model"):
        ...
        while queue:
            ...
            #if options.write_all_operators:
            if isinstance (pred, pddl.Action):
                all_operators_file.write("%s(%s)\n" % (pred.name, ", ".join(map(str, next_atom.args))))
